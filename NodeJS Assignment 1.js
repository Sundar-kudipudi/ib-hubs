...
app.post("/todos/", async (request, response) => {
  ...
  const validDate = format(new Date(dueDate), "yyyy-MM-dd");
  ...
  if (validCategory.includes(category) === false) {
    ...
    response.status(400);
    response.send("Invalid Todo Status");
  } else if (dueDate !== validDate) {
    response.status(400);
    response.send("Invalid Due Date");
  } else {
    const postTodoQuery = `
    INSERT INTO todo (id, todo, priority, status, category, due_date)
    VALUES (${id}, '${todo}', '${priority}', '${status}', '${category}', '${validDate}');`;
...
app.put("/todos/:todoId/", async (request, response) => {
  ...
  switch (true) {
    ...
    case requestBody.dueDate !== undefined:
      const validDate = format(new Date(requestBody.dueDate), "yyyy-MM-dd");
      if (requestBody.dueDate !== validDate) {
        response.status(400);
        response.send("Invalid Due Date");
      } else {
